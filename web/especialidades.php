...
<table width="880" height="0" align=center border="0" class=boxcircular>
  ...
  <br />
	...
	$codigoInformatica = "codigoInformatica";
	$codigoConstrucciones = "codigoConstrucciones";
	$codigoElectromecanica = "codigoElectromecanica";
	
    if ($especialidad == "informatica") {
		echo $codigoInformatica;
	}else{
		if ($especialidad == "construcciones") {
			echo $codigoConstrucciones;
		}else{
			if ($especialidad == "electromecanica") {
			echo $codigoElectromecanica;
		}else{
		$pagina = "Especialidades";
		$error = "Ninguna especialidad valida";
		echo ("<META HTTP-EQUIV='REFRESH' CONTENT='0;URL=".$url."".$urlError."?pagina=".$pagina."&error=".$error."'>");
	}
		}
	}
	?>
